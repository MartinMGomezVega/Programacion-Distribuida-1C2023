...
def crear_carrito():
    ...
    for carrito in CARRITOS:
        if carrito['user_id'] == user_id:
            abort(STATUS_CODE_BAD_REQUEST,"Ya existe un carrito para el usuario")
    carrito_id = len(CARRITOS) + 1
    nuevo_carrito = {"user_id": user_id, "carrito_id": carrito_id, "items": [], "operaciones":0}
...
def modificar_stock(carrito, operacion=TIPO_OPERACION.DESCONTAR_STOCK, items=None):
    if items:
        ...
        for item in carrito['items']:
            producto_id, cantidad = (item[PRODUCTO_ID], item[CANTIDAD]) if operacion == TIPO_OPERACION.REESTABLECER_STOCK else (item['producto_id'], item['cantidad'])
            producto = obtener_producto(producto_id)
